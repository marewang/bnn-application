...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(d =>
      ...
      d.nomorPegawai.toLowerCase().includes(keyword)
    );
  }, [q, data]);
          ...
          <tbody className="divide-y">
            ...
            {filtered.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50">
                <Td>{row.nama}</Td>
                <Td className="font-mono">{row.nomorPegawai}</Td>
                <Td>{toISODateInput(row.tmtPns)}</Td>
                <Td>{toISODateInput(row.riwayatTmtKgb)}</Td>
                <Td>{toISODateInput(row.riwayatTmtPangkat)}</Td>
                <Td className="font-medium">{toISODateInput(row.jadwalKgbBerikutnya)}</Td>
